...
def token_inventory(db: Session):
    ...
    return {"received": purchase, "issued": sale, "sales_return": sales_return, "purchase_return": purchase_return, "claimed": claimed, "available": purchase - purchase_return - sale + sales_return - claimed}
...
def apply_invoice_tokens(db: Session, invoice_type: str, invoice_id: uuid.UUID, items: list[dict]):
    ...
    for item in items:
        ...
        if include and value <= ZERO:
            raise HTTPException(status_code=422, detail="Token value must be configured for a token-bearing package")
...
def create_claim(db: Session, payload: TokenClaimCreate, user_id):
    ...
    with db.begin_nested():
        ...
        if payload.quantity > available:
            raise HTTPException(status_code=422, detail=f"Only {available} tokens are available to claim")
