...
from app.schemas.token import TokenClaimCreate, TokenClaimRead, TokenProductUpdate
...
def update_token_product(product_id: UUID, payload: TokenProductUpdate, db: Session = Depends(get_db)):
    product = db.get(Product, product_id)
    if not product:
        raise HTTPException(status_code=404, detail="Product was not found")
    if payload.token_enabled and payload.token_value <= 0:
        raise HTTPException(status_code=422, detail="Token value must be greater than zero when tokens are enabled")
    product.token_enabled = payload.token_enabled
    product.token_value = payload.token_value if payload.token_enabled else 0
    db.commit()
    db.refresh(product)
    return product
...
def apply_invoice_token_flags(invoice_type: str, invoice_id: UUID, payload: dict, db: Session = Depends(get_db)):
    if invoice_type not in {"sale", "purchase"}:
        raise HTTPException(status_code=422, detail="invoice_type must be sale or purchase")
